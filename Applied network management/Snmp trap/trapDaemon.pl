...
my @password = split('"', $lines[4]);
 ...
 or die "Connection Error: $DBI::errstr\n";
	...
	$sth = $dbh->prepare("CREATE TABLE IF NOT EXISTS `kp3_traps`
...
$time=time();
	...
	$trapVerify = $dbh->prepare("select * from `kp3_traps` where `IP`='$ip'");
	$trapVerify->execute;
	...
	$trapPrev = $dbh->prepare("select `STATUS`,`TIME`, `STATUS1` from `kp3_traps` where `IP`='$ip'");
	$trapPrev->execute;
	...
	$trapUpdate = $dbh->prepare("UPDATE `kp3_traps` SET  		`STATUS`='$status',`TIME`='$time',`PREVIOUS_STATUS`='$previousStatus',`PREVIOUS_TIME`='$previousTime',`STATUS1`='$status1',
`PREVIOUS_STATUS1`='$previousStatus1'  WHERE `IP` = '$ip'");
	...
	$trapNew = $dbh->prepare("insert into `kp3_traps` (IP,STATUS,TIME,STATUS1,PREVIOUS_STATUS1) VALUES ('$ip','$status','$time', '$status1','-')");	
	$trapNew->execute;
...
$trapDevice = $dbh->prepare("select * from `kp3_trapdevice`");
	$trapDevice->execute;
...
#Preparing FAIL status
	 $trapFail = $dbh->prepare("select * from `kp3_traps` WHERE `STATUS` = 'FAIL' AND `IP` = '$ipFAIL'");
	 $trapFail->execute;
...
#preparing DANGER status
	 ...
	 $trapDanger = $dbh->prepare("select * from `kp3_traps` WHERE `STATUS` = 'DANGER'");
	 $trapDanger->execute;
